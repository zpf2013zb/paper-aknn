...
void WriteQueryAppendFile(QueryPoint& queryPoint, const std::string& fileName)
{
    ofstream fout(fileName, ofstream::app);
    fout<<queryPoint.Ni<<" "<<queryPoint.Nj<<" "<<queryPoint.dist_Ni<<" "<<queryPoint.keywords<<" "<<queryPoint.k<<endl;
    fout.close();
}
...
ostream& operator<<(ostream& os, ResultSet& rs)
{
    for_each(rs.begin(), rs.end(), [&os](POI& poi){
...
int main(int argc,char** argv)
{
    ...
    //
    string configFileName = "config.prop"; //Global config, Local config
    ...
    while (true) {
        implcnt++;
        vector<unsigned long long> key = KeywordsGenerator::Instance().getConstantKeywords(1, numQueryKeywords);
        QueryPoint Q;
        genRandQ(Q, key[0], cr.getParameterK());
        cout<<"///////////////////////////////////////////////////////////////////////"<<endl;
        ...
        if (time2 < time0 && time2 < time1) {
            selectCount++;
            cout<<"Current Select "<<selectCount<<" Queries"<<endl;
            cout<<Q<<endl;
            WriteQueryAppendFile(Q, queryfile);
            if (selectCount >= cr.getParameterNumberOfQueryPoints()) {
                break;
            }
        }
    }
    ...
    fout<<"Query Point selected:"<<selectCount<<endl;
    fout<<endl;
